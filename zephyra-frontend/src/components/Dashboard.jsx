...
const Dashboard = ({ publicKey, onConnect, onDisconnect }) => {
  ...
  const { getRecentTransactions, lastUpdated } = useTransactions();
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        if (DEMO_MODE) {
          // Use mock data for demo mode
          setSavings({
            totalSavings: 1250.45,
            averageSavingsPercent: 26.5,
            monthlySavings: [
    ...
    fetchUserData();
  }, [publicKey, getRecentTransactions, lastUpdated]);
  ...
  const savingsChartData = {
    labels: savings?.monthlySavings?.map(item => item.month) || ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'],
    datasets: [
      {
        fill: true,
        label: 'Savings ($)',
        data: savings?.monthlySavings?.map(item => item.amount) || [12, 19, 25, 32, 45, 56],
                ...
                <div className="flex items-center bg-white bg-opacity-20 backdrop-filter backdrop-blur-sm rounded-xl p-4 border border-white border-opacity-20">
                  <div className="mr-3">
                    <div className="h-12 w-12 rounded-full bg-white flex items-center justify-center text-indigo-600 font-bold shadow-md">
                      {typeof publicKey === 'string' ? publicKey.substring(0, 1) : 'W'}
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="text-xs text-indigo-100 truncate w-32 md:w-48">
                      {typeof publicKey === 'string' 
                        ...
                        : DEMO_MODE ? 'GDEMO...DEMO7KEY' : 'Unknown Key'}
                    </div>
                  </div>
                </div>
                <button
              ...
              <div className="mb-4 space-y-4">
                ...
                {walletBalance?.MXN && (
                  <div className="flex justify-between items-center p-3 bg-yellow-50 rounded-lg">
                    ...
                    <span className="font-semibold text-yellow-700">${walletBalance.MXN}</span>
                  </div>
                )}
              </div>
            ...
            <div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                ...
                <div className="bg-gradient-to-br from-purple-500 to-purple-600 p-6 rounded-xl shadow-md text-white transform transition-all duration-300 hover:scale-105">
                  ...
                  <p className="text-3xl font-bold mt-2">
                    {savings?.annualYield?.toFixed(2) || '0.00'}%
                  </p>
            ...
            <>
              <div className="grid grid-cols-2 gap-4 mb-6">
                ...
                <div className="bg-gradient-to-br from-teal-500 to-teal-600 p-6 rounded-xl shadow-md text-white transform transition-all duration-300 hover:scale-105">
                  <div className="flex items-center justify-between">
                    <h3 className="text-sm font-medium text-teal-100">Average Yield</h3>
                    <div className="p-2 bg-white bg-opacity-20 rounded-lg">
                      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                        <path fillRule="evenodd" d="M12 7a1 1 0 110-2h5a1 1 0 011 1v5a1 1 0 11-2 0V8.414l-4.293 4.293a1 1 0 01-1.414 0L8 10.414l-4.293 4.293a1 1 0 01-1.414-1.414l5-5a1 1 0 011.414 0L11 10.586 14.586 7H12z" clipRule="evenodd"></path>
                      </svg>
                  ...
                  <div className="mt-2 text-xs text-teal-100">Annual percentage yield</div>
                </div>
              </div>
          ...
          {!publicKey ? (
            ...
            <div className="space-y-3">
              {recentTransactions.map((transaction, index) => (
                <div key={transaction.id || index} className="p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors duration-200">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                        transaction.type === 'remittance' ? 'bg-indigo-100 text-indigo-600' : 'bg-green-100 text-green-600'
                      }`}>
                        {transaction.type === 'remittance' ? (
                          ...
                          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                            <path fillRule="evenodd" d="M4 4a2 2 0 00-2 2v4a2 2 0 002 2h12a2 2 0 002-2v-4a2 2 0 00-2-2H4zm2 6a2 2 0 012-2h8a2 2 0 012 2v4a2 2 0 01-2 2h-2a2 2 0 01-2-2v-4zm6 4a2 2 0 100-4 2 2 0 000 4z" clipRule="evenodd"></path>
                          </svg>
                        )}
                      </div>
                      <div className="ml-3">
                        <p className="text-sm font-medium text-gray-800">
                          {transaction.type === 'remittance' ? 'Sent' : 'Received'} {transaction.sourceAsset} → {transaction.destinationAsset}
                        </p>
                        <p className="text-xs text-gray-500">
                          {new Date(transaction.timestamp).toLocaleDateString()} • {transaction.memo || 'No memo'}
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className={`text-sm font-semibold ${transaction.type === 'remittance' ? 'text-red-600' : 'text-green-600'}`}>
                        {transaction.type === 'remittance' ? '-' : '+'}{transaction.sourceAmount} {transaction.sourceAsset}
                      </p>
                      <p className="text-xs text-gray-500">
                        {transaction.destinationAmount} {transaction.destinationAsset}
                      </p>
                    </div>
                  </div>
                </div>
